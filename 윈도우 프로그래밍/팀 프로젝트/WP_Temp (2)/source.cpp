...
void MakeUnit(List* list,int Unit_ID) {
	...
	*/
	...
	case ID_ENEMY_NAME4:
		...
		unit->skill[0] = SKILL_Monster_attack;
		unit->skill[1] = SKILL_Monster_allattack;
		unit->skill[2] = SKILL_Monster_healing;
		unit->skill[3] = noskill;
...
LPCTSTR lpszWindowName = L"Team Project: Enter the Abyss";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		effectBitmap[2] = (HBITMAP)LoadImage(g_hInst, L"Image\\Campfire.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
		effectBitmap[3] = (HBITMAP)LoadImage(g_hInst, L"Image\\Marker.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
		effectBitmap[4] = (HBITMAP)LoadImage(g_hInst, L"Image\\Piece.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
		...
		*/
		for (int i = 0; i < 3; i++) {
			enemyShape[i].px = size.right/2 + 175 + 150 * i;
			enemyShape[i].py = 300;
		...
		ssystem->createSound("Music\\move.mp3", FMOD_LOOP_OFF, 0, &sound2); //--- 2번 사운드 생성 및 설정	//이동	(1초)
		ssystem->createSound("Music\\gun.mp3", FMOD_LOOP_OFF, 0, &sound3); //--- 2번 사운드 생성 및 설정	//총성	(1초)
		ssystem->createSound("Music\\slash.wav", FMOD_LOOP_OFF, 0, &sound4); //--- 2번 사운드 생성 및 설정	//칼질 (1초만 출력할것)
		ssystem->createSound("Music\\heal.mp3", FMOD_LOOP_OFF, 0, &sound5); //--- 2번 사운드 생성 및 설정	//회복 (0.8초)
	...
	if (menu == 0) {
		...
		case WM_PAINT:
			...
			*/
			...
			hFont = CreateFont(110, 0, 0, 0, FW_BOLD, 0, 0, 0, HANGUL_CHARSET, 0, 0, 0, 0, L"굴림");
			oldFont = (HFONT)SelectObject(mdc, hFont);
			{
				RECT rect = { 0,100,size.right,220 };
				_tcscpy(str, L"Enter the Abyss");
				//TextOut(mdc, 70, 100, str, lstrlen(str));
	...
	if (menu == 2) {
		...
		case WM_PAINT:
			...
			if (line == 10) {	
				...
				TransparentBlt(mdc, row + (1000.0 / (STAGELEN + 2)) * 10 - 10, col + 250 * 1 - 10, 170, 170, imgdc, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(255, 255, 255));
			}
			//화면 전환시
			if (DisplayChange == TRUE) {
				//imgdc = CreateCompatibleDC(hdc);
				...
				*/
				...
				SelectObject(imgdc, (HBITMAP)effectBitmap[4]);
				GetObject(effectBitmap[4], sizeof(BITMAP), &bmp);

				TransparentBlt(mdc, row + (1000.0 / (STAGELEN + 2)) * (line - 1), col + 250 * now,50, 50, imgdc, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(255, 255, 255));
				//Ellipse(mdc, row + (1000.0 / (STAGELEN + 2)) * (line - 1) , col + 250 * now , row + (1000.0 / (STAGELEN + 2)) * (line - 1) + 50, col + 250 * now + 50);
		...
		case WM_LBUTTONDOWN:
			if (MouseLock == FALSE && SelectCursor != 0) {				
				if ((SelectCursor-1) % STAGELEN == line) {
					...
					case 1:		//전투 페이즈로		
						...
						}
						{
							...
							for (int i = 0; i < 3; i++) {
								if (tmp != NULL) {
									enemy[i] = tmp;
									tmp->hp += 15 * (stage - 1);
									tmp->Maxhp += 15 * (stage - 1);
									tmp->AtkBuff += 5 * (stage - 1);									
						...
						channel2->stop();
						ssystem->playSound(sound2, 0, false, &channel2);
			...
			else if (line == 10 && MouseLock == FALSE) {
				if (CheckMouseRect(mx, my, row + (1000.0 / (STAGELEN + 2)) * 10 - 10, col + 250 * 1 - 10, row + (1000.0 / (STAGELEN + 2)) * 10 - 10 + 170, col + 250 * 1 - 10 + 170)) {
					MakeList(&enemylist);
					switch (stage) {
					case 1:
						MakeUnit(enemylist, ID_ENEMY_NAME4);
			...
			case 4:	//적의턴 시작까지 딜레이
				...
				//적의 행동패턴 시작
				{
					...
					case SKILL_Monster_attack:
					{
						...
						channel2->stop();
						ssystem->playSound(sound4, 0, false, &channel2);
						channel2->setVolume(0.2);
						SetTimer(hwnd, 10, 1000, NULL);
					}	
						break;
					...
					case SKILL_Monster_healing:
					{
						...
						channel2->stop();
						ssystem->playSound(sound5, 0, false, &channel2);
						channel2->setVolume(0.2);
						SetTimer(hwnd, 10, 1000, NULL);
					}
						break;
					...
					case SKILL_Monster_allattack:
					{
						...
						for (int i = 0; tmp != NULL; tmp = tmp->next) {
							Attack(tmp, 4 + turnlist->head->AtkBuff/2);
						}
					}
					channel2->stop();
					ssystem->playSound(sound4, 0, false, &channel2);
					...
					for (Unit* temp = userlist->head; temp != NULL;) {
						Unit* next = temp->next;
						if (temp->hp < 1) {
							ActDeleteUnit(turnlist, temp);
							if (temp->CLASS <= 4) {
								DeleteUnit(userlist, temp);
								Unit* tmp = userlist->head;
								for (int i = 0; tmp != NULL; tmp = tmp->next) {
									unit[i] = tmp;
									i++;
			...
			for (int i = 0; i < userlist->n; i++) {
				if (unit[i] != NULL) {				
					...
					if (turnlist->head == unit[i]) {
						...
						SelectObject(imgdc, (HBITMAP)effectBitmap[3]);
						GetObject(effectBitmap[3], sizeof(BITMAP), &bmp);

						//Rectangle(mdc, rect.left, rect.top, rect.right, rect.bottom);
						TransparentBlt(mdc, unitShape[i].px - 40 / 2, unitShape[i].py - 200 - (30 - animate > animate / 2 ? animate : 30 - animate), 80, 80, imgdc, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(255, 255, 255));
			...
			for (int i = 0; i < enemylist->n; i++) {
				if (enemy[i] != NULL) {
					...
					if (turnlist->head == enemy[i]) {
						...
						//Rectangle(mdc, rect.left, rect.top, rect.right, rect.bottom);
						TransparentBlt(mdc, enemyShape[i].px - 40 / 2, enemyShape[i].py - 200-(30-animate> animate/2? animate: 30 - animate), 80, 80, imgdc, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(255, 255, 255));
			...
			if (SelectCursor > userlist->n + enemylist->n && SelectCursor <= userlist->n + enemylist->n + 6 || selectSkill != -1) {				
				...
				if ((SelectCursor <= userlist->n + enemylist->n+4 && SelectCursor > userlist->n + enemylist->n)|| (selectSkill < 4 && selectSkill >=0)) {
					...
					case SKILL_Crusader_Smite:
						...
						wsprintf(str, L"대상에게 피해를 입히며 방어도를 얻습니다.", 7 + turnlist->head->AtkBuff);
						break;
					case SKILL_Crusader_HolyLance:
						/*
						if (SelectUnit->def > 0)
						...
						wsprintf(str, L"실드가 있는 대상에게 더 큰 피해를 입힙니다. 실드가 없다면 기본 피해를 대신 입힙니다.");
						break;
					case SKILL_Crusader_Zealous_Speech:
						/*
						...
						wsprintf(str, L"적 전체에게 방어도에 비례한 피해를 가합니다.");
						break;
					case SKILL_Crusader_Block:
						...
						wsprintf(str, L"아군 모두에게 보호막을 제공합니다.");
						break;

					//Vestal							
					case SKILL_Vestal_Mace:					
						wsprintf(str, L"대상에게 피해를 입힙니다.");
						break;
					case SKILL_Vestal_Lectionary:						
						wsprintf(str, L"대상의 공격력을 약화시킵니다");
						break;
					case SKILL_Vestal_Miracle:					
						wsprintf(str, L"아군 전체의 체력을 다소 회복합니다.");
						break;
					case SKILL_Vestal_Heal:
						//Heal(SelectUnit, 10 + turnlist->head->AtkBuff);
						wsprintf(str, L"대상의 체력을 대량 회복합니다.");
						break;
					...
					case SKILL_Highway_Pistol:
						//Attack(SelectUnit, 5 + turnlist->head->AtkBuff);
						wsprintf(str, L"대상에게 랜덤피해를 입힙니다.");
						break;
					case SKILL_Highway_Hipshot:
						//Attack(SelectUnit, 5 + turnlist->head->AtkBuff);
						wsprintf(str, L"대상의 보호막을 없앱니다.");
						break;
					case SKILL_Highway_Lunge:
						//Attack(SelectUnit, 5 + turnlist->head->AtkBuff);
						wsprintf(str, L"대상에게 피해를 입히고 맨 앞칸으로 이동합니다.");
						break;

					//Musketeer							
					case SKILL_Musketeer_rifle:
						//Attack(SelectUnit, 7 + turnlist->head->AtkBuff);
						wsprintf(str, L"대상에게 큰 피해를 입힙니다.");
						break;
					...
					case SKILL_Musketeer_Buckshot:
						...
						timer = 0;*/
						wsprintf(str, L"적 전체에게 멀수록 약해지는 공격을 가합니다.");
						break;
					case SKILL_Musketeer_Selfheal:
						//Attack(turnlist->head, 7);
						wsprintf(str, L"자신의 체력을 15% 회복합니다.");
						break;
					}
				}				
		...
		case WM_LBUTTONDOWN:
			if (SelectCursor != 0 && MouseLock == FALSE) {
				if (SelectCursor <= userlist->n && SelectCursor > 0 && selectSkill != -1) {
					...
					case SKILL_Crusader_Smite:
						Attack(SelectUnit, 7 + turnlist->head->AtkBuff);
						Shield(turnlist->head, 3+ turnlist->head->AtkBuff/2);
					...
					case SKILL_Crusader_Zealous_Speech:
						for (Unit* tmp = enemylist->head; tmp != NULL; tmp = tmp->next) {
							Attack(SelectUnit, 1 + turnlist->head->def/2);
						}
						break;					
						//Vestal							
					case SKILL_Vestal_Mace:
					...
					case SKILL_Vestal_Lectionary:
						//Attack(SelectUnit, 4);
						SelectUnit->AtkBuff -= 3;
						if (SelectUnit->AtkBuff < 0)
							SelectUnit->AtkBuff = 0;
					...
					case SKILL_Highway_Slash:
						...
						break;
					case SKILL_Highway_Pistol:
						Attack(SelectUnit, 2 + turnlist->head->AtkBuff + rand()%8);

						channel2->stop();
						ssystem->playSound(sound3, 0, false, &channel2);
						channel2->setVolume(0.2);
						SetTimer(hwnd, 10, 1000, NULL);
					...
					case SKILL_Highway_Hipshot:
						if (SelectUnit->def > 0) {
							SelectUnit->def = 0;
							//Attack(SelectUnit, 20 + turnlist->head->AtkBuff);
						}
						...
						break;
					case SKILL_Highway_Lunge:
						Attack(SelectUnit, 15 + turnlist->head->AtkBuff);
						{
							Unit* tmp = turnlist->head;
							Unit* first = unit[0];
							int turnmain = tmp->space;
							int selmain = first->space;

							tmp->space = selmain;
							first->space = turnmain;
					...
					case SKILL_Musketeer_rifle:
						...
						break;
					case SKILL_Musketeer_pistol:
						Attack(SelectUnit, 3 + turnlist->head->AtkBuff + rand()% 10);
					...
					*/
					for (Unit* temp = enemylist->head; temp!= NULL;) {
						Unit* next = temp->next;
						if (temp->hp < 1) {
							turnlist->head->AtkBuff++;
							ActDeleteUnit(turnlist, temp);
							if (temp->CLASS <= 4) {
								DeleteUnit(userlist, temp);
								Unit* tmp = userlist->head;
					...
					switch (turnlist->head->skill[selectSkill]) {				
					case SKILL_Musketeer_Selfheal:
						Heal(turnlist->head, turnlist->head->Maxhp*3/20);
					...
					case SKILL_Crusader_Block:
						//Attack(SelectUnit, 5 + turnlist->head->atk);
						for (Unit* tmp = userlist->head; tmp != NULL; tmp = tmp->next) {
							Shield(tmp, 5 + turnlist->head->AtkBuff/2);
						}
					...
					case SKILL_Vestal_Miracle:
						for (Unit* tmp = userlist->head; tmp != NULL; tmp = tmp->next) {
							Heal(tmp, 3 + turnlist->head->AtkBuff/2);
						}
		...
		case WM_PAINT:
			...
			//캐릭터 출력
			{
				...
				while (tmp != NULL) {
					...
					switch (tmp->CLASS) {
					case ID_USER_Crusader:
						TransparentBlt(mdc, 300 + charsize * i, 500-25, charsize, 260, imgdc, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(255, 255, 255));
						break;
					default:
						TransparentBlt(mdc, 300 +charsize * i, 500+25, charsize, 210, imgdc, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(255, 255, 255));
						break;
					}
					i++;
					tmp = tmp->next;
			...
			for (int i = 0; i < 3; i++) {
				int j = i + 1;
				int len = (size.right / 3);
				hPen = (HPEN)GetStockObject(BLACK_PEN);
				oldPen = (HPEN)SelectObject(mdc, hPen);
				hBrush = CreateSolidBrush(RGB(j * 70, j * 20, (j * 100) % 255));
				oldBrush = (HBRUSH)SelectObject(mdc, hBrush);
				RECT rect = { len * i, 200, len * (i + 1), 450 };
				Rectangle(mdc, len*i, 0, len* (i+1), 450);


				if (CheckMouseRect(mx, my, rect.left, rect.top, rect.right, rect.bottom)) {
					...
					AlphaBlend(mdc, rect.left, rect.top, rect.right, rect.bottom, imgdc, 0, 0, skillShape[i].width, skillShape[i].height, bf);
				...
				hFont = CreateFont(40, 0, 0, 0, FW_BOLD, 0, 0, 0, HANGUL_CHARSET, 0, 0, 0, 0, L"굴림");
				oldFont = (HFONT)SelectObject(mdc, hFont);
				...
				case 0:
					_tcscpy(str, L"파티 전체\n 공격력 + 3");
					//TextOut(mdc, len* i, 200, str, lstrlen(str));
					DrawText(mdc, str, lstrlen(str), &rect, DT_CENTER );
	...
	if (menu == 5) {
		...
		case WM_PAINT:
			...
			{	
				...
				for (int i = 0; tmp != NULL; tmp = tmp->next) {
					switch (unit[i]->CLASS) {
					case ID_USER_Crusader:
						SelectObject(imgdc, (HBITMAP)CrusaderBitmap[32]);
						GetObject(CrusaderBitmap[32], sizeof(BITMAP), &bmp);
						break;
					case ID_USER_Vestal:
						SelectObject(imgdc, (HBITMAP)VestalBitmap[32]);
						GetObject(VestalBitmap[32], sizeof(BITMAP), &bmp);
						break;
					case ID_USER_Highway:
						SelectObject(imgdc, (HBITMAP)HighwayBitmap[32]);
						GetObject(HighwayBitmap[32], sizeof(BITMAP), &bmp);
						break;
					case ID_USER_Musketeer:
						SelectObject(imgdc, (HBITMAP)MusketeerBitmap[32]);
						GetObject(MusketeerBitmap[32], sizeof(BITMAP), &bmp);
						break;
					}

					TransparentBlt(mdc, 400 - 70*i, 200 + 70*i, 170, 230, imgdc, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(255, 255, 255));
					i++;
				}			
				
				//캠프파이어
				SelectObject(imgdc, (HBITMAP)effectBitmap[2]);
				GetObject(effectBitmap[2], sizeof(BITMAP), &bmp);
				TransparentBlt(mdc, 550, 250, bmp.bmWidth, bmp.bmHeight, imgdc, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(255, 255, 255));

				RECT rect = { 550, 400, 890, 720 };
				TCHAR str[100];
				...
				wsprintf(str, L"Recover HP 20");
				DrawText(mdc, str, lstrlen(str), &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);

				SelectObject(mdc, oldFont);
				DeleteObject(hFont);
				//SetBkColor(mdc, OPAQUE);
		...
		case WM_LBUTTONDOWN:
			my = HIWORD(lParam);
			mx = LOWORD(lParam);
			if (CheckMouseRect(mx, my, 550, 250, 890, 720)) {
				Unit* tmp = userlist->head;			
				while (tmp != NULL) {
					Heal(tmp, 20);															
					tmp = tmp->next;
