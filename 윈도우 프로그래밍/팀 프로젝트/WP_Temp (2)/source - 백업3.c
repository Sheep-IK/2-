...
void MakeUnit(List* list,int Unit_ID) {
	...
	*/
	...
	case ID_ENEMY_NAME1:
		...
		unit->skill[0] = noskill;
		unit->skill[1] = noskill;
		unit->skill[2] = noskill;
...
void Shield(Unit* target, int shieldvalue) {
	int left = shieldvalue;
	target->def += left;
	return;
}

void Heal(Unit* target, int value) {
...
//유닛 비트맵 저장 핸들
HBITMAP CrusaderBitmap[33];	//0~30 = 애니메이션, 31: 초상화, 32: 휴식
HBITMAP VestalBitmap[33];
HBITMAP HighwayBitmap[33];
HBITMAP MusketeerBitmap[33];
HBITMAP BkBitmap;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (menu == 0) {
		...
		case WM_CREATE:
			...
			BkBitmap = (HBITMAP)LoadImage(g_hInst, L"Image\\Background.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
	...
	if (menu == 2) {
		...
		case WM_PAINT:
			...
			for (int i = 0; i < 3; i++) {
				...
				Rectangle(mdc, 100 + 300 * i, 100, 300 + 300 * i, 300);
				TCHAR str[100];
				switch (i) {
				case 0:
					wsprintf(str, L"전투 페이즈로");
					break;
				case 1:
					wsprintf(str, L"상점 페이즈로");
					break;
				case 2:
					wsprintf(str, L"휴식 페이즈로");
					break;
				}
				TextOut(mdc, 100 + 300 * i, 100, str, lstrlen(str));			
			...
			*/
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < STAGELEN; j++) {
					...
					if (SelectCursor == i * 10 + j + 1 && j - 1 >= line) {
						TransparentBlt(mdc, 50 + (1000.0 / STAGELEN)* j-10, 30 + 250 * i-10, 70, 70, imgdc, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(255, 255, 255));
					}
					else
						TransparentBlt(mdc, 50 + (1000.0/ STAGELEN) * j, 30 + 250 * i, 50, 50, imgdc, 0, 0, bmp.bmWidth, bmp.bmHeight, RGB(255,255,255));
				}
			}
		...
		case WM_MOUSEMOVE:
			if (MouseLock == FALSE) {
				...
				for (int i = 0; i < 3; i++) {
					int check = 0;
					for (int j = 0; j < STAGELEN; j++) {
						if (CheckMouseRect(mx, my, 50 + (1000.0 / STAGELEN) * j, 30 + 250 * i, 50 + (1000.0 / STAGELEN) * j + 50, 80 + 250 * i )) {
	...
	if (menu == 3) {
		...
		case WM_PAINT:
			...
			if (SelectCursor > 0 && SelectCursor < 5) {
				tmp = unit[SelectCursor - 1];
			}
			
			for (int i = 0; i < 6; i++) {
				if (i == selectSkill && ((SelectCursor > 0 && SelectCursor < 5 && turnlist->head == unit[SelectCursor - 1]) || SelectCursor == 0 || SelectCursor >= 5)) {
			...
			if (tmp != NULL && tmp->CLASS < 5) {				
				...
				Rectangle(mdc, 5, 650, (int)((double)5 + (double)140 * ((double)tmp->hp / (double)tmp->Maxhp)), 675);
			...
			//적군유닛에 커서 올릴시			
			tmp = NULL;
			if (SelectCursor > 4 && SelectCursor < 8) {
				tmp = enemy[SelectCursor - 5];
			}
			if (tmp != NULL) {
				switch (tmp->CLASS) {				
				case ID_ENEMY_NAME1:
			...
			if (MouseLock == FALSE) {
				...
				for (int i = 0; i < (13 + turnlist->n); i++) {
					...
					if (i < 4) {
						...
						rect = &turnShape[turnlist->n - 1 + 13 - i];
					}
					...
					if (i == 13 + turnlist->n - 1 ) { //&& SelectCursor != i + 1
						SelectCursor = 0;
						//InvalidateRect(hwnd, NULL, FALSE);
					}
				}
		...
		case WM_LBUTTONDOWN:
			if (SelectCursor != 0) {
				if (SelectCursor <= 4) {
				...
				else if (SelectCursor <= 7 && SelectCursor > 4 &&  selectSkill != -1) {			
					MouseLock = TRUE;
					SelectUnit = enemy[SelectCursor - 4];
					KillTimer(hwnd, 2);
					...
					default:
						...
						//Crusader
					case SKILL_Crusader_Smite:
						Attack(SelectUnit, 7 + turnlist->head->atk);
						break;
					case SKILL_Crusader_HolyLance:
						if (SelectUnit->def > 0)
							Attack(SelectUnit, 10 + turnlist->head->atk);
						else
							Attack(SelectUnit, 2 + turnlist->head->atk);
						break;
					case SKILL_Crusader_Zealous_Speech:
						for (Unit* tmp = enemylist->head; tmp != NULL; tmp = tmp->next) {
							Attack(SelectUnit, 3 + turnlist->head->atk);
						}
						break;
					case SKILL_Crusader_Block:
				...
				else if (SelectCursor <= 13) {
					selectSkill = SelectCursor - 8;
					if (SelectCursor == 13) {
						turnlist->head = turnlist->head->turnnext;
						selectSkill = -1;
					}
